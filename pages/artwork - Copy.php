...
<?php
$TITLE = "Djelo — Digitalna Galerija";
...
if (!$art) {
    echo "<h1>Djelo nije pronađeno</h1>";
    return;
}

// ✅ ispravna putanja slike (apsolutna u odnosu na folder projekta)
...
<div class="grid cols-3">
  ...
  <div class="card">
    ...
    <?php if ($art['cijena']): ?>
      <p class="price">Cijena: <strong><?= number_format($art['cijena'], 2, ',', '.') ?> KM</strong></p>
    <?php endif; ?>

    <p>Autor: <strong><?= htmlspecialchars(($art['ime'] ?? '') . ' ' . ($art['prezime'] ?? '')) ?></strong></p>
    <div class="notice">
      Certifikat autentičnosti dostupan pri kupovini.
      <a href="/galerija_sarajevo/authenticity-certificate">Saznaj više</a>.
    </div>
...
Zanima me dostupnost i cijena za "<?= htmlspecialchars($art['naslov']) ?>".
      ...
      <div style="margin-top:8px">
        <button type="submit" name="send_inquiry"
          style="background:var(--accent);border:none;padding:10px 14px;
                 border-radius:10px;font-weight:700;color:white;cursor:pointer;">
          Pošalji upit
        </button>
      </div>
    </form>
...
<p>
  <a class="badge" href="/galerija_sarajevo/umjetnici?id=<?= $art['umjetnik_id'] ?>">← Nazad na umjetnika</a>
</p>
