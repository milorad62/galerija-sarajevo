...
// Učitaj djela umjetnika (samo aktivna/objavljena ako ima kolona status)
$hasStatus = false;
...
if ($hasStatus) { $sql .= " AND status = 'approved'"; }
$sql .= " ORDER BY id DESC";

$st = $conn->prepare($sql);
$st->bind_param('i', $id);
$st->execute();
$artworks = $st->get_result()->fetch_all(MYSQLI_ASSOC);
$st->close();
?>
...
<?php if (!empty($artworks)): ?>
  <div class="grid cols-3">
    <?php foreach ($artworks as $art): ?>
      <div class="card">
        <a href="<?= htmlspecialchars($BASE_URL) ?>/djelo?id=<?= (int)$art['id'] ?>" style="text-decoration:none;color:inherit;">
          <div class="title"><?= htmlspecialchars($art['naslov'] ?: 'Bez naslova') ?></div>
          <img src="<?= htmlspecialchars($BASE_URL) ?>/uploads/<?= htmlspecialchars($art['slika'] ?: 'placeholder.jpg') ?>" style="max-width:100%;height:auto;border-radius:10px;" alt="Umjetnina">
          <?php if (!empty($art['opis'])): ?><p class="muted"><?= htmlspecialchars($art['opis']) ?></p><?php endif; ?>
          <?php if (!empty($art['cijena'])): ?><div class="muted"><?= number_format((float)$art['cijena'], 2, ',', '.') ?> KM</div><?php endif; ?>
        </a>
