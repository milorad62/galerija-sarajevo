...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if (!$email || !$pass) $error='Unesite e-mail i lozinku.';
  else {
    ...
    if ($u=$res->fetch_assoc()) {
      if (password_verify($pass, $u['lozinka'])) {
        $_SESSION['umjetnik_id']=(int)$u['id']; $_SESSION['umjetnik_ime']=trim(($u['ime']??'').' '.($u['prezime']??''));
        header('Location: /galerija_sarajevo/pages/dodaj_umjetninu.php'); exit;
      } else $error='Pogrešna lozinka.';
    } else $error='Korisnik ne postoji.';
  }
}
...
<div class="header"><strong>MojaGalerija.ba</strong><div><a href="/galerija_sarajevo/register.php">Registracija</a></div></div>
<div class="container"><div class="card" style="max-width:460px;margin:2rem auto;">
...
<div style="margin-top:.8rem;display:flex;gap:.5rem;"><button class="btn primary">Prijavi se</button><a class="btn" href="/galerija_sarajevo/">Nazad</a></div>
